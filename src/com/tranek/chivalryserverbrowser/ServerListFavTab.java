...
public class ServerListFavTab extends JPanel implements ServerListInterface {
	private final MainWindow mw;
	public ServerFilters sf = new ServerFilters();
	public RefresherFav refresher;
	...
	public void loadFilters() {
		...
		try {
			db.open(true);		
			SQLiteStatement st = db.prepare("CREATE TABLE IF NOT EXISTS filters_fav" +
					"(" +
			...
			st = db.prepare("SELECT COUNT(*) FROM filters_fav");
			try {
				st.step();
				if ( st.columnInt(0) > 0 ) {
					hasEntry = true;
			...
			if ( hasEntry ) {
				...
				int perspective = 0;
				int numthreads = 8;
				st = db.prepare("SELECT * FROM filters_fav WHERE id=1");
				try {
					st.step();
	...
	public void saveFilters() {
		...
		try {
			...
			st = db.prepare("INSERT OR REPLACE INTO filters_fav (id, name, type, minrank, maxrank, maxping, " +
