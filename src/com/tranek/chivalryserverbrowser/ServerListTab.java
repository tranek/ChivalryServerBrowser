...
public class ServerListTab extends JPanel implements ServerList {
	...
	public void loadFilters() {
		...
		try {
			db.open(true);		
			SQLiteStatement st = db.prepare("CREATE TABLE IF NOT EXISTS filters" +
					"(" +
			...
			st = db.prepare("SELECT COUNT(*) FROM filters");
			try {
				st.step();
				if ( st.columnInt(0) > 0 ) {
					hasEntry = true;
			...
			if ( hasEntry ) {
				...
				int perspective = 0;
				int numthreads = 8;
				st = db.prepare("SELECT * FROM filters WHERE id=1");
				try {
					st.step();
	...
	public void saveFilters() {
		...
		try {
			...
			st = db.prepare("INSERT OR REPLACE INTO filters (id, name, type, minrank, maxrank, maxping, " +
